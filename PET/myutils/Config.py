...
class Config(object):
    def __init__(self, filenames=None, kwargs=None):
        ...
        # Trainer configs
        self.num_steps = 300
        # self.eval_epoch_interval = 10000
    ...
    def update_kwargs(self, kwargs, eval=False):
        for (k, v) in kwargs.items():
            if k!='exp_name' and k!='exp_dir':
                if eval:
                    try:
    ...
    def set_exp_dir(self):
        ...
        if self.exp_dir is not None:
            ...
            self.save_config(os.path.join(self.exp_dir, "config.json"))
