...
def train(args):
   ...
   #* Log a few random examples:
    if rank <= 0:
        ...
        for index in range(3):
            ...
            # logger.info(f"{raw_eval_dataset[index]}")
   ...
   #* DataLoaders Creation:
    ...
    #* Tune other parameters around adapter
    ...
    if args.do_train:
        for epoch in range(args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                # return
                model.backward(loss)
                # if rank<=0: print(f'step {step}', flush=True)
                ...
                if step % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                    ...
                    if global_steps % args.eval_step == 0 and args.do_eval:     
                        if rank <= 0:
                        ...
                        if rank <= 0 and eval_score is not None: 
           ...
           # TODO: Evaluate the model after training per epoch----------------------
            ...
            if rank <= 0 and args.do_eval and score is not None: 
                ...
                if use_wandb: wandb.log({"score":score}, step=global_steps)
                # result_table.append({
                #     "epoch": epoch,
                #     "global_step": global_steps,
                #     "score": score,
