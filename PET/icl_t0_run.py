...
def train(args):
   ...
   # TODO: Begin Training ------------------------------------
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        masked_log_probs = eval_batch["labels_attention_mask"].unsqueeze(-1) * torch.log_softmax(logits, dim=-1)
        seq_token_log_probs = torch.gather(masked_log_probs, -1, eval_batch["labels"].unsqueeze(-1))
        seq_log_prob = seq_token_log_probs.squeeze(dim=-1).sum(dim=-1)
        seq_log_prob = seq_log_prob.view(eval_batch["targets"].size(0), -1) 
        predictions = seq_log_prob.argmax(dim=-1)

        eval_metric.add_batch(predictions=predictions, references=eval_batch["targets"],)
