...
# layer=2
layer=$3
# function=twolayer${layer}_selfattn_trainbtz${train_btz}
function=layer${layer}_selfattn_trainbtz${train_btz}
# function=twofnn
# function=allfnn
...
# other_params=(".*11.layer.*.SelfAttention.*"  ".*10.layer.*.SelfAttention.*")
...
$py -m torch.distributed.launch --nproc_per_node=$ngpu --master_port=$port \
    ...
    --tb_grads=$tb_grads \
    --reset_trainable_params=$reset_params \
