...
taskdata=wikisql #! different skg tasks
# taskdata=wikitq
# taskdata=compwebq #y
# taskdata=$3
# taskdata=webqsp #y
...
function=normal_trainbtz${train_btz} #! add additional function
# function=normal_tbgrads
# function=normal_lr${lr}_reduction${adapter_reduction}
# function=twofnn
# function=allfnn
# function=fivefnn
# function=selfattn_tbgrads
# function=allfnn_closeepo2_norecover
# function=allfnn_closeepo10_norecover
# function=debug
# other_params=(".*SelfAttention.*")
# other_params=(".*11.layer.*.DenseReluDense.*"  ".*10.layer.*.DenseReluDense.*" ".*9.layer.*.DenseReluDense.*" ".*8.layer.*.DenseReluDense.*" ".*7.layer.*.DenseReluDense.*")
...
# max_target=128
# max_target=64
echo Max Input Length $max_input 
echo Max Target Length $max_target
# padding=true
num_beams=4
padding=
...
exp+="${method}_${task_type}-task_${dataset_config}_ngpu${ngpu}_${model_name}"
exp+="_${function}_lr${lr}_run${runid}" #! add other functions
echo EXP: $exp
...
locallog=/home/v-yingxzhao/logs/$exp.log
log=${pet}/logs/$exp.log
# tbdir=/home/v-yingxzhao/tb_logs/$exp 
# azure_tbdir=${pet}/tb_logs/$exp 
tbdir=${pet}/tb_logs/$exp 
cache_dir=${pet}/cache_dir/$exp
mkdir -p ${pet}/cache_dir $cache_dir /home/v-yingxzhao/logs/
mkdir -p ${pet}/outputs $output ${pet}/logs $tbdir
# mkdir -p $azure_tbdir
